  ...
  let __updateSeen = false, __updateVer = '';
  function __applyUpdate(){
    try{ location.reload(); }catch(e){}
  }
  function __showUpdateBox(ver){
    __updateVer = ver || '';
    const ov = document.getElementById('updateBox');
    if(!ov){ __applyUpdate(); return; }          // box hi na ho to seedha reload
    const pr=document.getElementById('updProg'); if(pr) pr.classList.add('hidden');
    const bt=document.getElementById('updBtn');  if(bt) bt.classList.remove('hidden');
    const lt=document.getElementById('updLater'); if(lt) lt.classList.remove('hidden');
    ov.classList.remove('hidden');
  }
  // Profile se manual check. Background check chup-chaap rehta hai (kuch naya
  // na ho to kuch nahi bolta), par yahan user ne khud poochha hai — isliye
  // dono jawab dene padte hain: naya hai, ya pehle se latest ho.
  let __updChecking = false;
  function manualCheckUpdate(){
    if(__updChecking) return;
    if(!__BUILD){ showToast('⚠️ '+t('updCheckFail'),'warning'); return; }
    __updChecking = true;
    const arrow=document.getElementById('pfCheckUpdateArrow');
    const old = arrow ? arrow.textContent : '';
    if(arrow) arrow.textContent='⏳';
    fetch('/version.json?t='+Date.now(), { cache:'no-store' })
      .then(r => r.ok ? r.json() : null)
      .then(j => {
        __updChecking = false;
        if(arrow) arrow.textContent = old;
        if(!j || !j.version){ showToast('⚠️ '+t('updCheckFail'),'warning'); return; }
        if(j.version === __BUILD){ showToast('✅ '+t('upToDate'),'success'); return; }
        __updateSeen = true;              // background check dobara na tapke
        __showUpdateBox(j.version);
      })
      .catch(() => {
        __updChecking = false;
        if(arrow) arrow.textContent = old;
        showToast('⚠️ '+t('updCheckFail'),'warning');
      });
  }
  function closeUpdateBox(){
    const ov=document.getElementById('updateBox'); if(ov) ov.classList.add('hidden');
  }
  function __updSetProgress(done, total, labelKey){
    const fill=document.getElementById('updBarFill');
    const step=document.getElementById('updStep');
    if(fill) fill.style.width = Math.round((done/total)*100) + '%';
    if(step) step.textContent = t(labelKey);
  }
  // Progress asli kaam ka hai, dikhawa nahi — har step sach me kuch karta hai,
  // aur bar tabhi aage badhta hai jab wo step poora ho jaata hai.
  function startAppUpdate(){
    const bt=document.getElementById('updBtn');   if(bt) bt.classList.add('hidden');
    const lt=document.getElementById('updLater'); if(lt) lt.classList.add('hidden');
    const pr=document.getElementById('updProg');  if(pr) pr.classList.remove('hidden');

    const v = __updateVer ? ('?v='+encodeURIComponent(__updateVer)) : '';
    const steps = [
      // 1. Nayi files sach me utaar lo, taaki reload ke baad wait na ho
      ...
      //    warna vendor logos/photos dobara download honge bina zarurat ke.
      { key:'updStepClean', run: function(){
          if(!window.caches) return Promise.resolve();
          return caches.keys().then(function(ks){
            return Promise.all(ks.filter(function(k){ return k.indexOf('fbt-img') !== 0; })
                                 .map(function(k){ return caches.delete(k); }));
          }).catch(function(){});
        } },
      { key:'updStepReload', run: function(){ return Promise.resolve(); } }
    ];

    const total = steps.length;
    __updSetProgress(0, total, steps[0].key);
    (function next(i){
      if(i >= total){ __applyUpdate(); return; }
      steps[i].run().then(function(){
        __updSetProgress(i+1, total, steps[Math.min(i+1, total-1)].key);
        setTimeout(function(){ next(i+1); }, 250);   // bar ka jump dikhne do
