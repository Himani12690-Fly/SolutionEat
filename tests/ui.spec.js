...
test.describe('App loads', () => {
  ...
  test('bottom nav ke saare tabs maujood hain', async ({ page }) => {
    await openApp(page);
    // Bina ?v= wale URL pe app hamesha discovery (kitchen list) pe pehle khulta
    // hai — bottom nav wahan jaanbujh ke hidden hai. Kisi customer screen pe
    // jaate hi (goTo) hi nav dikhta hai.
...
test.describe('Icons', () => {
  ...
  test('nav icons ki actual width hai (render ho rahe hain)', async ({ page }) => {
    await openApp(page);
    // Bina ?v= wale URL pe app discovery pe khulta hai, jahan bottom nav
    // jaanbujh ke hidden hai — display:none ke andar sab getBoundingClientRect()
    // se width:0 milta hai. Kisi customer screen pe jaate hi nav dikhta hai.
    await goTo(page, 'land');
