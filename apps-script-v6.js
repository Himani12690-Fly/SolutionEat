...
// Apps Script cold-start: script kaafi der se hit nahi hua ho to agla request
// 1-3+ sec slow lagta hai (kam-visited pages jaise Subscriptions ispe zyada
// bhugatte hain). Isse hamesha "warm" rakhne ke liye: Apps Script editor →
// Triggers (⏰ icon) → Add Trigger → Function: keepWarm → Time-driven →
// Every 10 minutes → Save. Ek baar setup karna hai, fir background me chalta rahega.
function keepWarm() { try { loadVendors(); } catch (e) {} }
...
function setupSecrets() {
  PropertiesService.getScriptProperties().setProperties({
    ...
    FCM_PROJECT_ID   : 'flyingbirdstiffin',
    FCM_CLIENT_EMAIL : 'firebase-adminsdk-fbsvc@flyingbirdstiffin.iam.gserviceaccount.com',
    FCM_PRIVATE_KEY  : '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
  }, false);
  Logger.log('✅ Secrets save ho gaye — ab upar wali values code se hata do');
}

// Diagnostic — Step D verify karne ke liye. Run karke Execution log check karo:
// ✅ matlab FCM_PROJECT_ID/FCM_CLIENT_EMAIL/FCM_PRIVATE_KEY sahi se set hain.
function testFcmAuth() {
  const token = fcmAccessToken();
  if (token) Logger.log('✅ FCM auth OK — access token mila (length ' + token.length + ')');
  else Logger.log('❌ FCM auth failed — FCM_PROJECT_ID / FCM_CLIENT_EMAIL / FCM_PRIVATE_KEY check karo');
}

// "Invalid argument: key" wali error debug karne ke liye — Script Property me
// FCM_PRIVATE_KEY kaise stored hai wo check karta hai (bina value print kiye).
function debugFcmKey() {
  const k = secret('FCM_PRIVATE_KEY');
  Logger.log('length: ' + k.length);
  Logger.log('has real newline (\\n char): ' + (k.indexOf('\n') !== -1));
  Logger.log('has literal backslash-n text: ' + (k.indexOf('\\n') !== -1));
  Logger.log('starts with BEGIN header: ' + (k.indexOf('-----BEGIN PRIVATE KEY-----') === 0));
  Logger.log('ends with END footer: ' + (k.trim().lastIndexOf('-----END PRIVATE KEY-----') === k.trim().length - '-----END PRIVATE KEY-----'.length));
}
...
function aiContext() {
  ...
  const d = menu[day] || {};
  const meals = [];
  ['breakfast','lunch','dinner','meal4'].forEach(function(m){
    // meal4 opt-IN hai (baaki teen opt-OUT) — check ulta hai.
    const on = (m === 'meal4') ? (cfg.mealsEnabled && cfg.mealsEnabled.meal4 === true) : !(cfg.mealsEnabled && cfg.mealsEnabled[m] === false);
    if (!on) return;
    let item = '';
    try { item = (m === 'breakfast') ? String((d[m]||[])[0]||'') : String(((d[m]||{}).sabziOptions||[])[0]||''); } catch(e){}
    const label = (m === 'meal4') ? (cfg.meal4Label || 'meal4') : m;
    meals.push('- ' + label + ': ' + (item || 'not set') + ' | price Rs.' + ((cfg.prices||{})[m] || '?'));
  });
  // Cutoffs ab Admin Setup se configurable hain — hardcoded 9AM/3PM/10PM text yahan
  // STALE ho jaata agar admin badal de, aur AI customer ko galat time bata deta.
  const co = Object.assign({ lunch:540, dinner:900, breakfast:1320, meal4:1080 }, cfg.cutoffs || {});
  const clk = mins => { mins=((mins%1440)+1440)%1440; const h=Math.floor(mins/60), mi=mins%60; const ap=h>=12?'PM':'AM'; let h12=h%12; if(h12===0)h12=12; return h12+':'+String(mi).padStart(2,'0')+' '+ap; };
  const cutoffLine = 'ORDER CUTOFFS: breakfast by ' + clk(co.breakfast) + ' previous night, lunch by ' + clk(co.lunch) + ', dinner by ' + clk(co.dinner)
    + ((cfg.mealsEnabled && cfg.mealsEnabled.meal4 === true) ? (', ' + (cfg.meal4Label || 'meal4') + ' by ' + clk(co.meal4)) : '') + '.';
  return [
...
function askAI(p) {
  ...
  let who = 'guest';
  try { const s = getSessionCross(p.token, p.authVendorId); if (s && s.phone) who = s.phone; } catch(e){}
...
function rateVendor(p) {
  const s = getSessionCross(p.token, p.authVendorId);
...
function myRating(p) {
  const s = getSessionCross(p.token, p.authVendorId); if (!s) return { status:'invalid_session' };
  const rSh = ratingsSheet(); const rl = rSh.getLastRow();
...
function listVendorsForSuper() {
  ...
  return Object.keys(reg).map(function(id){
    const v = reg[id];
    // ⚠️ adminUser pehle yahan return nahi hota tha — isliye Edit form ka "Admin
    // Username" field HAMESHA khaali khulta tha (dikhta thik lagta tha, par asal
    // me kabhi actual value nahi dikhati thi). tgBotToken/tgChatId RAW nahi bhejte
    // (secrets hain, adminPass jaisa) — sirf configured hai ya nahi, wahi batao.
    return { vendorId:id, name:v.name || id, sheetId:v.sheetId || '', adminUser:v.adminUser || '', notifyEmail:v.notifyEmail || '',
      ...
      tgConfigured: !!(v.tgBotToken && v.tgChatId),
      rating: v.ratingCount ? Math.round((v.ratingSum / v.ratingCount) * 10) / 10 : 0,
...
function saveVendor(p) {
  ...
  try { provisionVendorSheet(String(p.sheetId).trim()); }
    ...
    // ID sahi ho, phir bhi 2 alag wajah se yahi generic error dikhta tha — ab dono
    // ko alag pehchaan ke actual reason + exact fix batate hain.
    const raw = String((e && e.message) || e);
    let hint;
    // "illegal spreadsheetid or key" Google ka generic "no access" hai (privacy ke
    // liye "not found" na bolke — file exist karti hai ya nahi wo bhi disclose nahi karte).
    // Isliye ise bhi permission-hint me route karo, sirf "permission"/"access" wale text pe nahi.
    if (/permission|access|illegal|invalid/i.test(raw)) {
      let owner = '';
      try { owner = Session.getEffectiveUser().getEmail(); } catch (e2) {}
      hint = owner
        ? 'Sheet share nahi hai. Us Sheet ko kholo → Share button → "' + owner + '" ko Editor access do, phir dobara try karo.'
        : 'Sheet share nahi hai us Google account ke saath jispe ye script chal raha hai. Sheet → Share → wo account Editor bana do.';
    } else if (/not found|entity was not found/i.test(raw)) {
      hint = 'Ye Sheet ID exist nahi karta. Sheet ka URL kholo, "/d/" ke baad wala poora ID (agle "/" tak) copy karo.';
    } else {
      hint = 'ID sahi hai? Aur sheet aapke hi Google account me honi chahiye.';
    }
    return { status:'error', message:'Sheet khul nahi payi. ' + hint + ' (' + raw.slice(0, 90) + ')' };
  }
  // ⚠️ Pehle p.tgBotToken/p.tgChatId khaali hone par SEEDHA blank likh dete the —
  // Super Admin ke vendor-edit form me ye fields kabhi thi hi nahi, isliye HAR save
  // (chahe sirf status toggle karne ke liye hi ho) in dono ko HAMESHA khaali kar
  // deta tha, chahe kisi ne pehle Sheet me manually bhare bhi ho. adminPass jaisa
  // hi pattern: khaali bheja gaya to purana jo bhi tha wahi rakho.
  const tgBotToken = p.tgBotToken ? String(p.tgBotToken).trim() : (existing ? (existing.tgBotToken || '') : '');
  const tgChatId = p.tgChatId ? String(p.tgChatId).trim() : (existing ? (existing.tgChatId || '') : '');
  const row = [id, safeCell(p.name || id), p.sheetId, safeCell(p.adminUser), passHash, p.notifyEmail || '', tgBotToken, tgChatId, safeCell(p.whatsapp || ''), p.status || 'Active', todayIST(), salt, String(p.scriptUrl || '').trim(),
...
const ORDERS_SHEET='Orders', MENU_SHEET='Menu', USERS_SHEET='Users', SESS_SHEET='Sessions', AUDIT_SHEET='Audit', CONFIG_SHEET='Config', PUSH_SHEET='PushTokens';
...
function ordersRawCached() {
  ...
  _ordersRawCache = (last < 2) ? [] : sh.getRange(2, 1, last - 1, 35).getValues();
  return _ordersRawCache;
}
function resetOrdersCache() { _ordersRawCache = null; }

// Login/header pe "Nest & Nosh" hardcoded tha har vendor ke liye — ab yahan se
// asli vendor ka naam + logo bhejte hain, frontend usse UI me laga deta hai.
// ⚠️ 'whatsapp' yahan se MISSING tha — frontend ka vendorWhatsapp() VENDOR_BRAND.whatsapp
// expect karta hai, aur wo hamesha undefined aata tha (kyunki backend kabhi bhejta hi
// nahi tha), isliye vendorWhatsapp() apna fallback (OWNER_WHATSAPP, ek hardcoded
// single number) HAR baar use karta — sab vendors ke customer orders isi EK number
// pe jaate the, chahe vendor ne apna alag WhatsApp registry me set kiya ho.
function vendorBrand() {
  const v = currentVendor() || {};
  return { name: v.name || 'Nest & Nosh', logo: v.logo || '', whatsapp: v.whatsapp || '' };
}
...
function doGet(e) {
  ...
  if (action === 'config') return json({ status:'success', config: readConfig(), vendor: vendorBrand() });
  if (action === 'bootstrap') { let pp=[]; try{ pp=publicPromos(); }catch(e){} return json({ status:'success', menu: readMenu(), config: readConfig(), promos: pp, vendor: vendorBrand() }); }
  if (action === 'publicstats') return json({ status:'success', stats: publicStats(q.date) });

  if (action === 'me') {
    const s = getSessionCross(q.token, q.authVendorId);
...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'savepushtoken') return json(savePushToken(p));
    if (action === 'testpush') {
      if (!checkAuth(p.user, p.pass)) return json({ status:'error', message:'Invalid credentials' });
      notifyAllSubscribers('✅ Test Notification', 'Push notifications are working!', {});
      return json({ status:'success' });
    }

    if (action === 'savemenu') {
      if (!checkAuth(p.user, p.pass)) return json({ status:'error', message:'Invalid credentials' });
      return json(saveMenu(p.day, p.dayData, p.notify === '1' || p.notify === true));
    }
    ...
    if (action === 'checkpromo') {
      const sCk = getSessionCross(p.token, p.authVendorId);
    ...
    if (action === 'validate') {
      if (!checkAuth(p.user, p.pass)) return json({ status:'error', message:'Invalid credentials' });
      return json({ status:'success', message:'Valid credentials' });
    }
...
// ── Discovery flow: cross-kitchen session resolve ──
// User kisi bhi kitchen (CURRENT_VENDOR_ID = jo select ki hai) se order/action kar
// sakta hai, PAR uska login-token us kitchen ki sheet me hota hai jahan usne SIGN-IN
// kiya tha (authVendorId). Isliye token pehle SELECTED kitchen ki apni sheet me
// dhoondo; na mile TABHI auth-kitchen ki sheet me — aur woh bhi sirf tab jab woh ek
// REAL, alag registered vendor ho.
// ⚠️ Milne ke turant baad CURRENT_VENDOR_ID WAPAS selected kitchen pe set hota hai,
// taaki order/sub/rating/myOrders sab USI selected kitchen ki sheet me jaaye — DEFAULT
// vendor ya login-kitchen me galti se kuch bhi na jaye (no mixing).
function getSessionCross(token, authVendorId) {
  if (!token) return null;
  let s = getSession(token);                 // 1) selected kitchen ki apni sheet
  if (s) return s;
  const aid = String(authVendorId || '').trim().toLowerCase();
  const reg = loadVendors();
  // Sirf ek REAL registered, ALAG kitchen ki sheet me jhaanko — warna kuch mat karo.
  // (blank/garbage authVendorId par DEFAULT vendor pe fallback KABHI nahi hoga.)
  if (!aid || !reg[aid] || aid === CURRENT_VENDOR_ID) return null;
  const selectedVid = CURRENT_VENDOR_ID;
  CURRENT_VENDOR_ID = aid;                    // 2) sirf TOKEN verify karne ke liye switch
  try { s = getSession(token); } catch (e) { s = null; }
  CURRENT_VENDOR_ID = selectedVid;           // 3) WAPAS selected kitchen pe — data yahin jaayega
  // uRow auth-kitchen ki Users sheet ka row hai; selected-kitchen sheet pe kabhi use na ho.
  if (s) s.uRow = null;
  return s;
}
...
function placeOrderAuthed(p) {
  const sess = getSessionCross(p.token, p.authVendorId);
  ...
  ['society','flatNo','note','name','lunchSabzi','dinnerSabzi','lunchRoti','dinnerRoti','lunchAddons','dinnerAddons','payment','lunchTiffin','dinnerTiffin','lunchTimeSlot','dinnerTimeSlot','breakfastTimeSlot','couponCode','meal4Sabzi','meal4Roti','meal4Addons','meal4Tiffin','meal4TimeSlot']
  ...
  try {
    ...
    // ⚠️ Yahan pehle notifyTelegram() bhi call hota tha — wo GLOBAL hardcoded
    // TG_BOT_TOKEN/TG_CHAT_ID use karta tha (CURRENT_VENDOR_ID ko ignore karke),
    // isliye HAR vendor ka order EK SHARED Telegram chat pe bhi jaata tha, sendOrderTelegram()
    // (neeche, jo currentVendor() ka apna tgBotToken/tgChatId use karta hai) ke
    // DOUBLE me. Hata diya — sendOrderTelegram() hi sahi, per-vendor tareeka hai.
...
function mealQtys(p) {
  const out = { breakfast: 0, lunch: 0, dinner: 0, meal4: 0 };
  ...
  out.meal4     = parseInt(p.meal4Qty, 10) || 0;
  return out;
}
...
function computeTotal(p) {
  ...
  if (Array.isArray(p.items) && p.items.length) {
    ...
    p.items.forEach(it => {
      ...
      if (['breakfast','lunch','dinner','meal4'].indexOf(meal) === -1) return;
...
function orderWindowError(p) {
  ...
  const cfg = readConfig();
  // Emergency close — frontend jaisa hi, YAHAN bhi enforce karo warna koi seedha
  // API hit karke bypass kar sakta hai jab kitchen turant band hai.
  if (cfg.tempClosed) return cfg.tempClosedMsg || 'The kitchen is temporarily closed. Please check back later.';
  ...
  if (mq.meal4     > 0 && !me.meal4) return (cfg.meal4Label || 'This meal') + ' is not available right now.';   // meal4 opt-IN hai — !me.meal4, not === false
  ...
  const q = mealQtys(p);
  const bq = q.breakfast, lq = q.lunch, dq = q.dinner, m4q = q.meal4;
  if (bq + lq + dq + m4q <= 0) return 'Please select at least one meal.';

  // Admin Setup se configurable (CFG.cutoffs) — pehle yahan 540/900/1320 hardcoded
  // the, frontend ka mealsAvail() ke bilkul alag se, dono kabhi sync me rakhne ki
  // zimmedari kisi ek jagah nahi thi. Ab dono readConfig() ka SAME cutoffs padhte hain.
  const dc = { lunch: 540, dinner: 900, breakfast: 1320, meal4: 1080 };
  const cutoffs = Object.assign({}, dc, cfg.cutoffs || {});
  if (off === 0) {
    if (bq > 0) return 'Same-day breakfast is not available.';
    if (lq > 0 && mins >= cutoffs.lunch) return 'Today\'s lunch booking has closed.';
    if (dq > 0 && mins >= cutoffs.dinner) return 'Today\'s dinner booking has closed.';
    if (m4q > 0 && mins >= cutoffs.meal4) return 'Today\'s ' + (cfg.meal4Label || 'meal4').toLowerCase() + ' booking has closed.';
  }
  if (off === 1) {
    if (bq > 0 && mins >= cutoffs.breakfast) return 'Tomorrow\'s breakfast booking has closed.';
    // lunch/dinner/meal4 for tomorrow: always open (till their own next-day cutoffs)
  }
  ...
  const meals = { breakfast: bq, lunch: lq, dinner: dq, meal4: m4q };
...
const MEAL_QTY_COL = { breakfast: 7, lunch: 8, dinner: 12, meal4: 30 };
function mealQtyForDate(deliveryDate, meal, excludeRow) {
  const qtyCol = MEAL_QTY_COL[meal] || 12;
  const data = ordersRawCached();
  let sum = 0;
  for (let i = 0; i < data.length; i++) {
    const row = i + 2;
...
function ordersSheet() {
  const sh = sheetWithHeaders(ORDERS_SHEET, [
    ...
    'Breakfast Time','Lunch Time','Dinner Time','Lunch Tiffin','Dinner Tiffin',
    'Payment Status','Promo','Delivery Type','Meal Status',
    'Meal4 Qty','Meal4 Sabzi','Meal4 Roti','Meal4 Addons','Meal4 Tiffin','Meal4 Time'
  ]);
...
function tgOrderMsg(p, name, total, tag) {
  ...
  if (parseInt(p.meal4Qty,10)) { const cc=readConfig(); meals.push((cc.meal4Emoji||'🍿') + ' ' + (cc.meal4Label||'Meal4') + ' ×' + p.meal4Qty + (p.meal4Tiffin ? ' (' + p.meal4Tiffin + ')' : '')); }
...
function saveOrder(p, phone, name, total, promoStr) {
  ...
  ordersSheet().appendRow([
    ...
    '',   // col29 MealStatus — naye order pe khaali (parseMealStatus() handle karta hai), IS position ko meal4 columns se PEHLE preserve karna zaroori hai
    wq.meal4, safeCell(pick('meal4','sabzi','meal4Sabzi')), safeCell(pick('meal4','roti','meal4Roti')), safeCell(p.meal4Addons || ''), safeCell(p.meal4Tiffin || ''), safeCell(p.meal4TimeSlot || '')
  ]);
}
...
const MEAL_KEYS = ['breakfast','lunch','dinner','meal4'];
...
function setMealStatus(p) {
  ...
  const active = MEAL_KEYS.filter(function(m){ return Number(sh.getRange(row, MEAL_QTY_COL[m]).getValue()) > 0; });
...
function rowToOrder(r, rowNum) {
  ...
  return {
    ...
    meal4Qty: r[29] || 0, meal4Sabzi: r[30] || '', meal4Roti: r[31] || '', meal4Addons: r[32] || '', meal4Tiffin: r[33] || '', meal4TimeSlot: r[34] || '',
  ...
  function allOrders() {
    try {
      const cached = CacheService.getScriptCache().get('orders_all_' + CURRENT_VENDOR_ID);
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed._cached && (Date.now() - parsed._cached < 30000)) {
          return parsed.data;
        }
      }
    } catch(e) {}
    ...
    const data = sh.getRange(2, 1, lastRow - 1, 35).getValues();
    const result = data.map((r, i) => rowToOrder(r, i + 2));
    
    try {
      CacheService.getScriptCache().put('orders_all_' + CURRENT_VENDOR_ID, 
        JSON.stringify({ data: result, _cached: Date.now() }), 60);
    } catch(e) {}
...
// ═══════════ CUSTOMER PUSH NOTIFICATIONS (Firebase Cloud Messaging) ═══════════
// ⚠️ Real Web Push (RFC 8291/8292 — VAPID JWT + aes128gcm payload encryption)
// needs ECDSA signing and AEAD encryption that Apps Script's Utilities class has
// NO support for. FCM is used instead: it does that crypto on Google's servers,
// and Apps Script only needs to authenticate as a Google service account, which
// Utilities.computeRsaSha256Signature() DOES support — this is Google's own
// documented pattern for calling Google APIs from Apps Script.
//
// One-time setup needed OUTSIDE this file (cannot be done from code):
//   1) console.firebase.google.com → Create a project (free tier is enough).
//   2) Project Settings → Cloud Messaging → Web Push certificates → Generate key
//      pair → copy the "Key pair" value → this is FIREBASE_VAPID_KEY.
//   3) Project Settings → General → Your apps → Add app → Web → copy the config
//      object (apiKey/projectId/messagingSenderId/appId) → paste into index.html's
//      FIREBASE_CONFIG placeholder (frontend, near the top of the <script>) — these
//      are public/client-safe values, not secrets.
//   4) Project Settings → Service Accounts → Generate new private key → downloads a
//      JSON file. From it, run setupSecrets() (or the Script Properties UI) with:
//        FCM_PROJECT_ID       = the JSON's "project_id"
//        FCM_CLIENT_EMAIL     = the JSON's "client_email"
//        FCM_PRIVATE_KEY      = the JSON's "private_key" (paste exactly, with \n's)
function b64url(bytes) {
  return Utilities.base64EncodeWebSafe(bytes).replace(/=+$/, '');
}
function fcmAccessToken() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('fcm_token');
  if (cached) return cached;
  const clientEmail = secret('FCM_CLIENT_EMAIL');
  const privateKey = secret('FCM_PRIVATE_KEY');
  if (!clientEmail || !privateKey) return '';   // configure nahi kiya → chup-chaap skip (order flow kabhi block na ho)
  ...
  const toSign = b64url(Utilities.newBlob(JSON.stringify(header)).getBytes()) + '.' + b64url(Utilities.newBlob(JSON.stringify(claims)).getBytes());
  const sigBytes = Utilities.computeRsaSha256Signature(toSign, privateKey);
  const jwt = toSign + '.' + b64url(sigBytes);
  ...
  if (!j.access_token) { Logger.log('FCM auth fail: ' + resp.getContentText()); return ''; }
  try { cache.put('fcm_token', j.access_token, Math.min(3300, (j.expires_in || 3600) - 60)); } catch (e) {}
  return j.access_token;
}
// Ek device ko push bhejo. Return: 'ok' | 'invalid' (token dead — DB se hata do) | 'error' (retry-able, token na hatao)
function sendPushToToken(fcmToken, title, body, data) {
  const projectId = secret('FCM_PROJECT_ID');
  const accessToken = fcmAccessToken();
  ...
  if (/UNREGISTERED|NOT_FOUND|InvalidRegistration/i.test(txt)) return 'invalid';
  Logger.log('FCM send fail (' + code + '): ' + txt.slice(0, 200));
  return 'error';
}
// Vendor ke SAARE subscribed customers ko push bhejo — best-effort, ek fail se
// baaki na ruken. Dead tokens (UNREGISTERED) sheet se apne aap saaf ho jaate hain.
function notifyAllSubscribers(title, body, data) {
  try {
    const sh = pushTokensSheet();
    const last = sh.getLastRow();
    if (last < 2) return;
    const rows = sh.getRange(2, 1, last - 1, 2).getValues();
    const deadRows = [];
    rows.forEach(function (r, i) {
      const token = String(r[1] || ''); if (!token) return;
      const result = sendPushToToken(token, title, body, data);
      if (result === 'invalid') deadRows.push(i + 2);
    });
    for (let i = deadRows.length - 1; i >= 0; i--) sh.deleteRow(deadRows[i]);
  } catch (e) { /* push kabhi bhi order/admin flow ko block na kare */ }
}
function savePushToken(p) {
  const sess = getSessionCross(p.token, p.authVendorId); if (!sess) return { status: 'invalid_session' };
  const fcmToken = String(p.fcmToken || '').trim();
  if (!fcmToken) return { status: 'error', message: 'Token missing' };
  const sh = pushTokensSheet();
  const last = sh.getLastRow();
  if (last >= 2) {
    const vals = sh.getRange(2, 1, last - 1, 2).getValues();
    for (let i = 0; i < vals.length; i++) {
      if (String(vals[i][1]) === fcmToken) { sh.getRange(i + 2, 1).setValue(sess.phone); return { status: 'success' }; }   // same device, phone updated — re-link
    }
  }
  sh.appendRow([sess.phone, fcmToken, new Date()]);
  return { status: 'success' };
}
function sendTestPush() {
  notifyAllSubscribers('✅ Test Notification', 'Push notifications are working!', {});
  Logger.log('Bhej diya — subscribed device pe check karo.');
}
...
function sendOrderTelegram(p, phone, name, total) {
  ...
  if (parseInt(p.meal4Qty, 10))     { const cc=readConfig(); tg += (cc.meal4Emoji||'🍿') + ' ' + (cc.meal4Label||'Meal4') + ' × ' + p.meal4Qty + ' — ' + (p.meal4Sabzi || '') + '\n'; }
...
function sendOrderEmail(p, phone, name, total) {
  ...
  if (parseInt(p.meal4Qty,10)) { const cc=readConfig(); body += row((cc.meal4Emoji||'🍿') + ' ' + (cc.meal4Label||'Meal4') + ' × ' + p.meal4Qty, 'Sabzi: ' + p.meal4Sabzi + '<br>Roti: ' + p.meal4Roti + '<br>Add-ons: ' + p.meal4Addons); }
...
function sendDailyDigest() {
  // ⚠️ VENDORS kahin defined nahi hai (sirf VENDORS_SEED/loadVendors() hain) —
  // har trigger run pe ReferenceError, digest kabhi kisi vendor ko jaata hi nahi tha.
  Object.keys(loadVendors()).forEach(function(vid){
...
function sendDailyDigestForVendor() {
  ...
  const cfgD = readConfig();
  const meal4Line = (cfgD.meal4Emoji || '🍿') + ' <b>' + (cfgD.meal4Label || 'Meal4') + ':</b> ';
  const EXTRA_MEALS = ['lunch','dinner','meal4'];
  const sum = { bQty: 0 };
  EXTRA_MEALS.forEach(m => sum[m] = { qty:0, sabzi:{}, butter:0, extraRoti:0, dahi:0, extraSabzi:0 });
  active.forEach(o => {
    sum.bQty += Number(o.breakfastQty) || 0;
    EXTRA_MEALS.forEach(m => {
  ...
  EXTRA_MEALS.forEach(m => {
    const s = sum[m];
    if (m === 'meal4' && !s.qty) return;   // meal4 zyaadatar vendors use nahi karte — 0 ho to digest me line hi mat dikhao
    const lbl = m === 'lunch' ? '☀️ <b>Lunch:</b> ' : (m === 'dinner' ? '🌙 <b>Dinner:</b> ' : meal4Line);
    kitchen += '<li>' + lbl + s.qty + ' tiffin — ' + sabziList(s.sabzi);
  ...
  orders.forEach(o => {
    ...
    if (Number(o.meal4Qty)) meals.push((cfgD.meal4Emoji||'🍿') + ' ' + o.meal4Sabzi + ' ×' + o.meal4Qty + ' (' + o.meal4Roti + ')' + (o.meal4Addons && o.meal4Addons !== 'None' ? ' +' + o.meal4Addons : ''));
...
const strListMenu = (v) => Array.isArray(v) ? v.map(s => String(s).slice(0, 60)).filter(Boolean).slice(0, 20) : [];
// Ek din ka data validate/clean karo. Poora din missing/adhoora ho to null (caller
// decide karega kya karna hai — whole-week merge me purana din rakh lo, single-day
// save me seedha reject karo).
function sanitizeMenuDay(d) {
  if (!d || typeof d !== 'object') return null;
  const breakfast = strListMenu(d.breakfast);
  const lunchOk = d.lunch && typeof d.lunch === 'object';
  const dinOk = d.dinner && typeof d.dinner === 'object';
  const m4Ok = d.meal4 && typeof d.meal4 === 'object';
  const lunch = { sabziOptions: strListMenu(lunchOk ? d.lunch.sabziOptions : []), fixedItems: strListMenu(lunchOk ? d.lunch.fixedItems : []) };
  const dinner = { sabziOptions: strListMenu(dinOk ? d.dinner.sabziOptions : []), fixedItems: strListMenu(dinOk ? d.dinner.fixedItems : []) };
  // meal4 OPTIONAL hai — mealsEnabled.meal4 default OFF hai, isliye zyada vendors
  // ise kabhi bharenge hi nahi. breakfast/lunch/dinner jaisa REQUIRED banaya to
  // sabko is din ka save fail hone lagta (jo meal4 use hi nahi karte unke liye bhi).
  const meal4 = { sabziOptions: strListMenu(m4Ok ? d.meal4.sabziOptions : []), fixedItems: strListMenu(m4Ok ? d.meal4.fixedItems : []) };
  const complete = breakfast.length && lunch.sabziOptions.length && lunch.fixedItems.length && dinner.sabziOptions.length && dinner.fixedItems.length;
  return complete ? { breakfast: breakfast, lunch: lunch, dinner: dinner, meal4: meal4 } : null;
}
// ⚠️ Pehle Admin "Save Menu" ek hi button se POORE HAFTE ka menu bhej deta tha, chahe
// admin ne sirf EK din edit kiya ho — Admin ka apna in-memory snapshot (panel khulte
// waqt liya gaya) baaki 6 dino ko OVERWRITE kar deta tha. Do admin/tabs alag-alag din
// edit kar rahe hon to jo BAAD me save kare, doosre ka din wapas purani cached copy se
// silently reset ho jaata — "kaam nahi kar raha" jaisa lagta tha. Ab sirf EK din
// save hota hai, aur Sheet se HAMESHA FRESH (uncached) poora menu padh ke usi din ko
// merge karte hain — baaki dino ko kabhi touch nahi karte.
function saveMenu(day, dayData, notify) {
  if (MENU_DAYS.indexOf(String(day || '')) < 0) return { status: 'error', message: 'Invalid day' };
  const clean = sanitizeMenuDay(dayData);
  if (!clean) return { status: 'error', message: 'Is din ka menu adhoora hai — breakfast, lunch (sabzi + fixed) aur dinner (sabzi + fixed) sab bharo.' };
  const full = readMenuUncached() || {};
  full[day] = clean;
  const ss = getSS();
  let sh = ss.getSheetByName(MENU_SHEET);
  if (!sh) sh = ss.insertSheet(MENU_SHEET);
  sh.getRange(1, 1).setValue(JSON.stringify(full));
  ...
  resetMenuCache();
  // Har save par push NAHI — admin ek din ka menu 3-4 baar edit/save karte hue
  // dubaara-dubaara push nahi bhejna chahta. Admin panel me ek explicit "Notify
  // customers" checkbox se hi ye bhejta hai (opt-in, spam se bachne ke liye).
  if (notify) { try { notifyAllSubscribers('📋 Menu Updated', 'Check out today\'s updated menu!', { type: 'menu_updated' }); } catch (e) {} }
  return { status: 'success', menu: full };
  return { status:'success' };
}
...
function defaultVariants() {
  ...
  return {
    ...
    meal4: [ mk('std','Snacks',60,['🍿 Chef\'s choice']) ]   // simple 1-variant default, jaisa breakfast — admin naam/price/items khud badal sakta hai
  };
}
// Clean/validate an incoming variants object; ensures ids unique per meal.
function sanitizeVariants(v) {
  const out = {};
  ['breakfast','lunch','dinner','meal4'].forEach(m => {
...
function defaultConfig() {
  return {
    prices: { breakfast:30, lunch:80, dinner:80, meal4:60, tiffinMini:60, extraRotiPlain:12, extraRotiButter:16, dahi:20, extraSabzi:30 },
    ...
    closedDates: [], capacity: { breakfast:0, lunch:0, dinner:0, meal4:0 },
    // Cutoff times, minutes-since-midnight IST. lunch/dinner/meal4 apply to the
    // SAME delivery day; breakfast applies to the NIGHT BEFORE (order tomorrow's
    // breakfast by this time tonight). Pehle ye sab hardcoded the — 540=9:00 AM,
    // 900=3:00 PM, 1320=10:00 PM, 1080=6:00 PM.
    cutoffs: { lunch:540, dinner:900, breakfast:1320, meal4:1080 },
    // Sudden/emergency close — planned closedDates se ALAG: wo ek specific calendar
    // date block karta hai, ye ek turant "sab kuch band karo abhi" switch hai (jaise
    // gas leak, staff na hona) — admin jab tak khud ON->OFF na kare, saara ordering
    // (aaj + future sab) band rahega.
    tempClosed: false, tempClosedMsg: '',
    variants: defaultVariants(),
    banners: { breakfast:'', lunch:'', dinner:'', meal4:'' },   // optional meal banner images
    ...
    mealsEnabled: { breakfast:true, lunch:true, dinner:true, meal4:false },   // kaunsa meal customer ko dikhe — meal4 default OFF, admin explicitly opt-in kare
    // 4th (custom, admin-nameable) meal — 'meal4' hamesha internal key hai; naam/emoji
    // Setup se configurable. Ye request pe add hua ("sirf 3 hi option hai, chautha
    // add karna hai") — poora ordering/pricing/status pipeline lunch/dinner jaisa hi
    // treat karta hai isko (variants, dahi/extra-roti/extra-sabzi addons, apna cutoff).
    meal4Label: 'Snacks', meal4Emoji: '🍿',
    // ── B2B corporate mode ──
...
function readConfigUncached() {
  ...
  try {
    ...
    if (c.variants && c.variants.lunch) {
      ...
      variants.meal4[0].price = prices.meal4;
    }
    return {
      prices: prices,
      ...
      cutoffs: Object.assign({}, d.cutoffs, c.cutoffs || {}),
      tempClosed: (c.tempClosed === true),
      tempClosedMsg: String(c.tempClosedMsg || '').trim(),
      variants: variants,
      banners: Object.assign({ breakfast:'', lunch:'', dinner:'', meal4:'' }, c.banners || {}),
      ...
      mealsEnabled: {
        ...
        // ⚠️ meal4 baaki teen se ALAG hai — opt-IN (default OFF), opt-OUT nahi. Purane
        // saved configs me ye key kabhi nahi hogi, aur us case me FALSE hi rehna chahiye
        // (warna sabhi purane vendors ko achanak ek khaali "Snacks" meal dikhne lagega).
        meal4: !!(c.mealsEnabled && c.mealsEnabled.meal4 === true)
      },
      meal4Label: String(c.meal4Label || d.meal4Label).slice(0, 30),
      meal4Emoji: String(c.meal4Emoji || d.meal4Emoji).slice(0, 8),
...
function saveConfig(cfg) {
  ...
  // Cutoffs: minutes-since-midnight, 0..1439. Galat/missing value pe default rakho.
  const cutoffs = {};
  Object.keys(d.cutoffs).forEach(k => {
    let v = parseInt((cfg.cutoffs || {})[k], 10);
    cutoffs[k] = (isNaN(v) || v < 0 || v > 1439) ? d.cutoffs[k] : v;
  });
  const tempClosed = (cfg.tempClosed === true);
  const tempClosedMsg = String(cfg.tempClosedMsg || '').slice(0, 200).trim();
  const meal4Label = String(cfg.meal4Label || d.meal4Label).slice(0, 30).trim() || d.meal4Label;
  const meal4Emoji = String(cfg.meal4Emoji || d.meal4Emoji).slice(0, 8).trim() || d.meal4Emoji;
  const clean = { prices: prices, township: township, societies: societies, deliveryNear: dNear, deliveryFar: dFar, farSocieties: farSoc, closedDates: closedDates, capacity: capacity, cutoffs: cutoffs, tempClosed: tempClosed, tempClosedMsg: tempClosedMsg, meal4Label: meal4Label, meal4Emoji: meal4Emoji, upiId: upiId, upiName: upiName, fssai: fssai, whatsappAuto: (cfg.whatsappAuto !== false), mealsEnabled: { breakfast: !(cfg.mealsEnabled && cfg.mealsEnabled.breakfast === false), lunch: !(cfg.mealsEnabled && cfg.mealsEnabled.lunch === false), dinner: !(cfg.mealsEnabled && cfg.mealsEnabled.dinner === false), meal4: !!(cfg.mealsEnabled && cfg.mealsEnabled.meal4 === true) }, companies: companies, homeEnabled: homeEnabled, officeEnabled: officeEnabled };
  ...
  resetConfigCache();
  // Emergency-close TOGGLE hone par (dono direction — band ya wapas khula) subscribed
  // customers ko push bhejo. Har config save par NAHI — sirf jab tempClosed state
  // genuinely BADLA ho, warna admin baar-baar Setup save kare to spam ho jaayega.
  if (cur.tempClosed !== clean.tempClosed) {
    try {
      if (clean.tempClosed) notifyAllSubscribers('🚨 Kitchen Closed', clean.tempClosedMsg || 'We are temporarily not taking orders.', { type: 'kitchen_closed' });
      else notifyAllSubscribers('✅ Kitchen is Open Again', 'We are back and taking orders!', { type: 'kitchen_open' });
    } catch (e) {}
  }
  return { status:'success' };
}
...
function uploadImage(p) {
  ...
  function buildStats() {
    try {
      const cached = CacheService.getScriptCache().get('stats_' + CURRENT_VENDOR_ID);
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed._cached && (Date.now() - parsed._cached < 15000)) {
          return parsed;
        }
      }
    } catch(e) {}
    ...
    orders.forEach(o => {
      ...
      res.total.count++; 
      res.total.revenue += amt;
      if (o.deliveryDate === t) { 
        res.today.count++; 
        res.today.revenue += amt; 
      }
      if (o.deliveryDate >= weekAgoStr && o.deliveryDate <= t) { 
        res.week.count++; 
        res.week.revenue += amt; 
      }
    });

    res.recent = orders.filter(o => o.status !== 'Cancelled').slice(-10).reverse().map(o => ({
      time: o.time, name: o.name, society: o.society, flat: o.flat, 
      total: o.total, payment: o.payment, status: o.status, deliveryDate: o.deliveryDate
    }));

    try {
      CacheService.getScriptCache().put('stats_' + CURRENT_VENDOR_ID, JSON.stringify(res), 60);
    } catch(e) {}
...
// ⚠️ Ye trigger kisi incoming request ke bina chalta hai, isliye CURRENT_VENDOR_ID
// apne module-level default (DEFAULT_VENDOR_ID) par hi reh jaata tha — matlab is
// function ka poora kaam sirf DEFAULT vendor ke liye hota tha, kisi bhi doosre
// vendor ka subscription KABHI auto-order nahi bana. Ab saare registered vendors
// pe loop karte hain; ek vendor ka data/sheet issue baaki vendors ka run na roke.
function runDailySubscriptions() {
  const savedVendorId = CURRENT_VENDOR_ID;
  try {
    Object.keys(loadVendors()).forEach(function(vid){
      try {
        CURRENT_VENDOR_ID = vid;
        runDailySubscriptionsForVendor();
      } catch (e) {
        audit('SUB_DAILY_VENDOR_FAIL', vid, String((e && e.message) || e).slice(0, 200));
      }
    });
  } finally {
    CURRENT_VENDOR_ID = savedVendorId;
  }
}
function runDailySubscriptionsForVendor() {
  const sh = subsSheet(); const last = sh.getLastRow();
  if (last < 2) return;
  ...
  data.forEach(r => {
    ...
    try { p.phone = phone; sendTelegram(tgOrderMsg(p, r[1], total, '🔁 SUBSCRIPTION ORDER')); } catch (e) {}
  });
}